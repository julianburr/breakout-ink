const { SPAWN_FREQUENCY, SPAWN_DURATION, WIDTH } = require('./constants');
...
function applyEffectEnd (state, effect) {
  switch (effect.id) {
    case 'tiny_paddle':
      return {
        ...state,
...
function spawnEffects (state) {
  ...
  if (state.activeEffects.length) {
    state.activeEffects.forEach((effect) => {
      const duration = effect.duration === 0 ? 1000 : effect.duration;
